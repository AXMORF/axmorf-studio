...
import {runM7Evidence} from "../m7-gps/evidence";
...
import {freezeGpsM8Inputs} from "./freeze-inputs";
...
export const M8_REPRESENTATIVE_FRAMES = [
  ...
  1018, 1030, 1280, 1409, 1420, 1421, 1432, 1433, 1445, 1580, 1715,
  1730,
] as const;
...
export const inspectM8FinalPreviewTechnical = async ({
  ...
  const [{stderr: loudnessOutput}, {stderr: samplePeakOutput}, ffmpegVersion, ffprobeVersion] =
    await Promise.all([
      runTool(
        "ffmpeg",
        [
...
const collectM8FinalPreviewEvidence = async (rootDir: string) => {
  ...
  return createFinalPreviewEvidence({
    ...
    resourceCatalogFingerprint:
      frozen.assemblyCatalog.catalogFingerprint,
