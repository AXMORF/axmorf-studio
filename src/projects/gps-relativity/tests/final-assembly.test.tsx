...
test("GPS final assembly statically binds current Scene, global, and source identities", () => {
...
test("M8 runtime uses static imports and no executable data or external systems", async () => {
  const source = await readFile(
    new URL("../final-assembly-data.ts", import.meta.url),
    "utf8",
  );
  assert.doesNotMatch(
    source,
...
test("GlobalVisual renderer is frame-driven caption-safe and not a generic DSL", async () => {
  const source = await readFile(
    new URL("../global-visual/GlobalVisualLayers.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    source,
    /animation|transition|Caption|subtitle|layers\.map|componentId|modulePath|auto.*layout/i,
  );
});
