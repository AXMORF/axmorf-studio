...
import {validateComicDesignSystem} from "../../scripts/m9-product/comic-design-system";
import {assertM9MasteringMeasurements} from "../../scripts/m9-product/final-evidence";
import {
...
export const M9_FAIL_CLOSED_CASES = [
  ["product-source-checksum", "source-packet", "Story/source receipt, assembly, approval"],
  ["unsupported-marketing-claim", "source-packet", "source gate and StoryCheck"],
  ["story-beat-order-or-meaning-id", "story", "timing, coverage, registry, assembly"],
  ["authored-tts-text", "story", "seal, timing, captions, narrative"],
  ["voice-source-bytes", "source-packet", "source receipt, provider attempt, seal"],
  ["prompt-transcript-or-canonical-wav", "source-packet", "high-fidelity generation gate"],
  ["forbidden-voice-control", "source-packet", "provider adapter gate"],
  ["voice-profile-mode-or-parameters", "source-packet", "StoryCheck, provider attempt, seal"],
  ["complete-wav-byte", "sealed-narration", "seal, timing, downstream identities"],
  ...
  ["registry-import-descriptor-duration", "final-assembly", "baseline and assembly"],
  ...
  ["preview-byte-truncate-frame", "preview-evidence", "final evidence and approval"],
...
const expectIdentityDrift = (caseId: string, current: unknown, mutation: unknown) => {
  assert.notDeepEqual(mutation, current);
  assert.notEqual(
    createFingerprint({namespace: `m9-fail-closed-${caseId}`, version: 1, value: current}),
    createFingerprint({namespace: `m9-fail-closed-${caseId}`, version: 1, value: mutation}),
  );
};
...
const probeSourcePacket = async (caseId: string) => {
  ...
  if (caseId === "product-source-checksum") {
    ...
    assert.notEqual(checksum(mutation), receiptEntry("sources/product-source.json").checksum);
    return;
  }
...
const probeStory = async (caseId: string) => {
  ...
  if (caseId === "story-beat-order-or-meaning-id") {
    [mutation.beats[0], mutation.beats[1]] = [mutation.beats[1]!, mutation.beats[0]!];
    assert.notEqual(computeStoryFingerprint(mutation), computeStoryFingerprint(story));
    return;
  }
...
const probeSealedNarration = async (caseId: string) => {
  ...
  expectSchemaMutationRejected(SealedNarrationManifestSchema, current, (mutation) => {
...
const probeFinalAssembly = async (caseId: string) => {
  ...
  expectSchemaMutationRejected(FinalAssemblyPlanSchema, current, (mutation) => {
    if (caseId === "registry-import-descriptor-duration") {
      mutation.rendererRegistryFingerprint = `sha256:${"1".repeat(64)}` as never;
...
const probeShotcraft = async (caseId: string) => {
  ...
  if (caseId === "renderer-exact-binding") {
    ...
    assert.doesNotMatch(source.replaceAll("AdaptedShot", "DetachedShot"), /AdaptedShot/u);
    return;
  }
...
const probeScene = async (caseId: string) => {
  if (caseId === "scene-coverage") {
    ...
    expectSchemaMutationRejected(SceneCoverageMapSchema, coverage, (mutation) => {
...
const probeGlobal = async (caseId: string) => {
  if (caseId === "global-visual-ownership") {
    const current = GlobalVisualPlanSchema.parse(
      await readJson("src/projects/product-comic-vertical/global-visual-plan.json"),
    );
    expectSchemaMutationRejected(GlobalVisualPlanSchema, current, (mutation) => {
      (mutation as unknown as Record<string, unknown>).captionOwner = "GlobalVisual";
    });
    return;
  }
  const current = GlobalSoundPlanSchema.parse(
    await readJson("src/projects/product-comic-vertical/global-sound-plan.json"),
  );
...
const probePreviewEvidence = async (caseId: string) => {
  ...
  expectSchemaMutationRejected(FinalPreviewEvidenceSchema, current, (mutation) => {
    if (caseId === "preview-byte-truncate-frame") {
      ...
      mutation.media.contactSheet.checksum = `sha256:${"5".repeat(64)}` as never;
    }
  });
};
...
const probeApproval = async (caseId: string) => {
  if (caseId === "approval-missing") {
    assert.equal(FinalPreviewApprovalSchema.safeParse(undefined).success, false);
    return;
  }
  ...
  expectSchemaMutationRejected(FinalPreviewApprovalSchema, current, (mutation) => {
...
test("persisted M9 fail-closed matrix is complete current and fingerprinted", async () => {
  ...
  assert.ok(matrix.cases.every((entry: {status: string}) => entry.status === "pass"));
  const {matrixFingerprint, ...input} = matrix;
  assert.equal(
    matrixFingerprint,
    createFingerprint({namespace: "m9-fail-closed-matrix", version: 1, value: input}),
  );
});
...
test("M9 generalization report separates reuse fixes project-local work and proposals", async () => {
  ...
  for (const category of Object.values(report.categories) as {
    ...
    for (const entry of category) {
      for (const sourcePath of entry.sourcePaths ?? []) {
        const sourceStat = await stat(join(rootDir, sourcePath));
        assert.ok(sourceStat.isFile() || sourceStat.isDirectory());
      }
    }
  }
  const allIds = Object.values(report.categories).flatMap((entries) =>
    (entries as {capabilityId: string}[]).map(({capabilityId}) => capabilityId),
  );
  ...
  for (const proposal of report.categories.promotionCandidate) {
    ...
    for (const source of [...proposal.gpsSources, ...proposal.productSources]) {
      assert.ok((await stat(join(rootDir, source.path))).isFile());
      assert.equal(
        checksum(Uint8Array.from(await readFile(join(rootDir, source.path)))),
        source.checksum,
      );
    }
  }
