...
} from "../../scripts/m9-product/final-evidence";
...
test("M9 final review frames are automatic ordered complete and bounded", () => {
  ...
  assert.deepEqual(M9_REVIEW_FRAMES, [...M9_REVIEW_FRAMES].sort((a, b) => a - b));
  ...
  for (const boundary of [580, 1083, 1316, 1816, 2373, 3103, 3651, 4212, 4703]) {
...
test("M9 final evidence binds current full preview contact sheet technical facts and review", async () => {
  ...
  assert.equal(evidence.media.representativeStills.length, M9_REVIEW_FRAMES.length);
  assert.deepEqual(
  ...
  assert.deepEqual(
    {
      ...
      audio: [evidence.technical.audioCodec, evidence.technical.sampleRate, evidence.technical.channelLayout],
...
test("M9 final review is current pass-only and cannot represent user approval", async () => {
  ...
  const review = await validateM9FinalAssemblyReview({rootDir, rawReview: raw});
