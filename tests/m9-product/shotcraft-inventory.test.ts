...
const inventory = (items = [inventoryItem()]) => {
  ...
  return {...input, inventoryFingerprint: computeShotcraftFingerprint("inventory", input)};
};
...
const coverage = (items: readonly unknown[]) => {
  ...
  return {...input, coverageFingerprint: computeShotcraftFingerprint("coverage", input)};
};
...
const coverageItem = (overrides: Record<string, unknown> = {}) => ({
  ...
  reason: "The three-stage line reveal makes the product boundary legible without inventing a feature.",
...
test("inventory fails closed on count, duplicate identity, preview and checksum drift", () => {
  ...
  assert.throws(() =>
    validateInventoryDocument(inventory([inventoryItem({styleKey: "draw-svg-trace"}), inventoryItem()])),
  );
  assert.throws(() =>
    validateInventoryDocument(inventory([inventoryItem({preview: null})])),
  );
  assert.throws(() =>
    validateInventoryDocument({...inventory(), inventoryFingerprint: digest("9")}),
  );
});
...
test("inventory distinguishes library revision from commit and models demo resolution modes", () => {
  assert.throws(() =>
    validateInventoryDocument(
      inventory([inventoryItem({libraryRevision: "d4915443232e89527fdc9d7e79f132ba411fc440"})]),
    ),
  );
  for (const status of ["ambiguous", "missing", "special-template"] as const) {
    const exactDemo = {
      ...
      status,
      entryPath: status === "missing" ? null : inventoryItem().exactDemo.entryPath,
    };
    assert.doesNotThrow(() =>
...
test("inventory rejects unsafe closure paths, unversioned packages and missing license records", () => {
  ...
  for (const exactDemo of [
    ...
    {...base, barePackages: [{packageName: "react", exactVersion: "latest"}]},
  ]) {
    assert.throws(() =>
      validateInventoryDocument(inventory([inventoryItem({exactDemo})])),
    );
  ...
  assert.throws(() =>
    validateInventoryDocument(inventory([inventoryItem({licenses: incompleteLicenses})])),
  );
});
...
test("selected exact staging proves every localized closure path exists as a regular file", async () => {
  ...
  try {
    ...
    await mkdir(join(rootDir, "src/projects/product-comic-vertical/scenes/product-reveal/reference"), {
      recursive: true,
    });
    await writeFile(join(rootDir, destination), "export const proof = true;\n", "utf8");
    const stagingInput = {
      schemaVersion: 1,
      selectedExact: [
        {
    ...
    await assert.doesNotReject(() => validateTask5Staging({rootDir, staging}));
...
test("committed M9 inventory covers the frozen 104/161/161 corpus and exact selection closure", async () => {
  ...
  await assert.doesNotReject(() =>
    validateTask5Staging({rootDir: repositoryRoot, staging: JSON.parse(staging)}),
  );
});
