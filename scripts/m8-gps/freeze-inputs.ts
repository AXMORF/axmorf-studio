...
import {generateGpsGlobalAudio} from "./generate-global-audio";
...
export const buildGpsM8FrozenInputs = async (rootDir: string) => {
  const [baseCatalog, overlay, timing, render, sealedNarration, narrativeReport] =
    await Promise.all([
      readJson(
        rootDir,
  ...
  const baseIds = new Set(baseCatalog.entries.map((entry) => entry.descriptor.id));
  ...
  const byRole = new Map(overlayAssets.map((asset) => [asset.mediaRole, asset]));
  const receiptById = new Map<string, (typeof globalAudioReceipt.assets)[number]>(
    globalAudioReceipt.assets.map((asset) => [asset.resourceId, asset]),
  );
  const soundAssets = (
    ["cross-scene-ambience", "global-bgm"] as const
  ).map((role) => {
  ...
  const finalAssemblyInput = FinalAssemblyPlanInputSchema.parse({
    ...
    scenePackageFingerprints: [
      ...sceneBranch.scenePackageFingerprints,
    ].sort(),
...
if (
  ...
  freezeGpsM8Inputs({rootDir: process.cwd(), mode}).catch((error: unknown) => {
