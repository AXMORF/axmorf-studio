      ...
      .array(
        z
          .object({
            ...
            localPath: z
              .string()
              .startsWith("public/projects/gps-relativity/global-audio/"),
            synthesisId: z.enum([
...
const synthesize = (
  synthesisId: "gps-orbital-ambience-v1" | "gps-low-density-clock-bed-v1",
  sampleIndex: number,
) => {
  ...
  if (synthesisId === "gps-low-density-clock-bed-v1") {
    ...
    const clockBreath =
      0.82 + 0.18 * Math.sin(2 * Math.PI * 0.125 * time + 0.7);
    return (
...
const createCanonicalWav = (
  ...
  for (let index = 0; index < SAMPLE_FRAME_COUNT; index += 1) {
    const normalized = Math.max(
      -1,
      Math.min(1, synthesize(synthesisId, index)),
    );
...
const writeOrCheckBytes = async ({
  ...
  if (current !== null) {
    throw new Error(
      "M8 global audio writer refuses to overwrite unknown bytes.",
    );
  }
