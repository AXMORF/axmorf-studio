...
1. Inspect the current callable native child tools and their documented limits: child creation, completion (wait-any or synchronous batch return), and slot release. Use discoverable host tool search when available. A CLI on PATH, an installed plugin, a new chat, or a shell subprocess
