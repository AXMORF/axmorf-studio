...
import { computeRenderSpecFingerprint } from "../../../contracts/auto-check";
import { computeStoryFingerprint } from "../../../contracts/generation-input";
import { RenderSpecSchema } from "../../../contracts/render";
import { StorySpecSchema } from "../../../contracts/story";
import Renderer from "../scenes/workflow-input/Renderer";
...
const readSceneJson = async (relativePath: string) =>
  JSON.parse(
    await readFile(new URL(relativePath, sceneRoot), "utf8"),
  ) as unknown;
...
test("workflow-input freezes its Story timing style Catalog and continuity identity", async () => {
...
test("workflow-input binds the frozen inspiration-only provenance and one approved PCM cue", async () => {
  ...
  assert.equal(sha256(new Uint8Array(cueBytes)), cuePair.descriptor.checksum);
});
...
test("workflow-input plans one local Shot, stays visual-only, and packages the fixed 557-frame Beat", async () => {
  ...
  const rendererSource = await readFile(
    new URL("Renderer.tsx", sceneRoot),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    rendererSource,
    /(?:CaptionLayer|<Audio|staticFile\(|fetch\(|https?:\/\/|animation\s*:|transition\s*:)/u,
  );
  assert.doesNotMatch(
    shotSource,
    /(?:fetch\(|https?:\/\/|animation\s*:|transition\s*:)/u,
  );
