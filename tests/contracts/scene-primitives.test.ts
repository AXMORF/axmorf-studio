...
test("scene identifiers are stable slugs rather than paths or module identities", () => {
  assert.equal(
    SceneRendererIdSchema.parse("renderer-opening"),
    "renderer-opening",
  );
...
test("scene local frames and ranges reject unsafe negative empty and overflowing values", () => {
  ...
  const range = SceneLocalFrameRangeSchema.parse({
    startFrame: 3,
    endFrame: 10,
  });
  assert.deepEqual(assertSceneLocalRangeWithinDuration(range, 10), range);
  assert.throws(() =>
    SceneLocalFrameRangeSchema.parse({ startFrame: 3, endFrame: 3 }),
  );
  assert.throws(() =>
    SceneLocalFrameRangeSchema.parse({ startFrame: 4, endFrame: 3 }),
  );
...
test("scene and shot frame helpers use exact local offsets and fail outside half-open ranges", () => {
  ...
  assert.throws(() =>
    toSceneLocalFrame(Number.MAX_SAFE_INTEGER + 1, beatRange),
  );
});
