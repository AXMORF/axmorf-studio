...
const storyBeat = story.beats.find(({meaningId}) => meaningId === "workflow-input");
if (storyBeat === undefined) {
  throw new Error("workflow-input StoryBeat is missing.");
}

const durationInFrames = task.timingBeat.endFrame - task.timingBeat.startFrame;

export const WorkflowInputAuthoringComposition: FC = () => {
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={438} durationInFrames={8} layout="none">
        <Audio
          src={staticFile(
            "projects/product-comic-vertical/scene-audio/workflow-input/source-check-tick.wav",
          )}
          volume={0.28}
        />
      </Sequence>
...
export default WorkflowInputAuthoringComposition;
