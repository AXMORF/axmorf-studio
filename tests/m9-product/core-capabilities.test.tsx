  ...
  assert.deepEqual(anchors.anchors, [
    {
      ...
      sceneLocalFrame: 170,
      purpose: "VideoBrief、StorySpec、NarrationSpec 与 RenderSpec 四份规格完成逐层生成并稳定持有。",
    },
    {
      eventId: "semantic-chunks-linked",
      sceneLocalFrame: 248,
      purpose: "ttsChunks 作为按语义、语气与朗读节奏创作的单元接入系统，明确不按标点机械拆分。",
    },
    {
      eventId: "pcm-sealed",
      sceneLocalFrame: 324,
      purpose: "声音生成结果封存为唯一时间权威 PCM，蓝色 identity thread 在同帧接入累计采样边界。",
    },
    {
      eventId: "absolute-frames-derived",
      sceneLocalFrame: 398,
      purpose: "累计整数 sample-frame 边界推导出绝对帧，形成可复算的 timing stack。",
    },
    {
      eventId: "verified-stack-ready",
      sceneLocalFrame: 456,
      purpose: "四份规格、封存 PCM 与绝对帧保持 current，ScenePackage 和最终装配只显示其系统位置。",
    },
  ]);
});
...
test("core-capabilities Renderer reveals a readable causal stack without media or captions", async () => {
  ...
  const {default: Renderer} = await import(
    "../../src/projects/product-comic-vertical/scenes/core-capabilities/Renderer"
  );
  const baseProps = {
  ...
  const frames = [24, 170, 260, 405, 472].map((sceneFrame) =>
    renderToStaticMarkup(
      createElement(Renderer, {...baseProps, sceneFrame}),
    ),
  );
