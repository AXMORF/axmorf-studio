...
void main() {
  ...
  float primaryField = 0.0;
  for (int i = 0; i < 50; i++) {
    if (i >= iBallCount) {
      break;
    }
    primaryField += getMetaBallValue(iMetaBalls[i].xy, iMetaBalls[i].z, coord);
  }

  float cursorField = getMetaBallValue(mouseW, iCursorBallSize, coord);
  float total = primaryField + cursorField;
  float f = smoothstep(-1.0, 1.0, (total - 1.3) / min(1.0, fwidth(total)));

  vec3 cFinal = vec3(0.0);
  if (total > 0.0) {
    float alpha1 = primaryField / total;
    float alpha2 = cursorField / total;
...
const hash33 = ([x, y, z]: [number, number, number]): [
  number,
  number,
  number,
] => {
...
export const MetaBallsPrimitive: FC<MetaBallsPrimitiveProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const cursor =
      enableMouseInteraction && pointerInsideRef.current
        ? liveCursorRef.current
        : automatedCursor;
    ...
    state.program.uniforms.iMouse.value.set(
      renderedCursor.x,
      renderedCursor.y,
      0,
    );
