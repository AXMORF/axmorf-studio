...
const storyBeat = story.beats.find(({meaningId}) => meaningId === "product-reveal");
if (storyBeat === undefined) {
  throw new Error("product-reveal StoryBeat is missing.");
}

const durationInFrames = task.timingBeat.endFrame - task.timingBeat.startFrame;

export const ProductRevealAuthoringComposition: FC = () => {
  ...
  return (
    <AbsoluteFill>
      ...
      <Sequence from={80} durationInFrames={9} layout="none">
        <Audio
          src={staticFile(
            "projects/product-comic-vertical/scene-audio/product-reveal/ink-close-snap.wav",
          )}
          volume={0.28}
        />
      </Sequence>
...
export default ProductRevealAuthoringComposition;
