...
const sceneRoot =
  "src/projects/product-comic-vertical/scenes/call-to-action";
const audioPath =
...
test("call-to-action task input binds the current sealed timing and final continuity", async () => {
  ...
  assert.ok(storyBeat && previousBeat && timingBeat && styleEntry && audioEntry);
...
test("call-to-action source excludes promotion claims runtime services and CSS animation", async () => {
  ...
  assert.match(
    root,
    /id="ProductComicVertical-call-to-action-Authoring"/u,
  );
});
