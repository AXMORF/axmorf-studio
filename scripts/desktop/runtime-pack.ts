...
export const runRuntimePackCli = async (args: readonly string[]) => {
  ...
  if (command === "check" || command === "inventory") {
    ...
    const expectedArchitecture = DesktopDarwinArchitectureSchema.parse(
      architectureIndex === -1 ? process.arch : args[architectureIndex + 1],
    );
    const manifest = await verifyRuntimePack({
      runtimePackRoot: value(args, "--runtime-pack"),
      expectedPlatform: args.includes("--allow-cross-platform")
        ? "darwin"
        : process.platform,
      expectedArchitecture,
    });
    process.stdout.write(
      `${JSON.stringify(command === "inventory" ? manifest.files : manifest)}\n`,
    );
    return;
  }
  if (command === "build") {
