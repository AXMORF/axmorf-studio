...
Task 7 首次 final-check 复核暴露两处现场兼容缺口，先按原 ownership 最小修复再继续聚合：

1. `core-capabilities/selected-resources.json` 是十个 Scene 中唯一缺少顶层 `schemaVersion: 1`
   的 wrapper；交回原 6D Scene owner，先给专属测试补 Red，再只修该 JSON，确认 package/media
   identity 不变；精确 stage 该 JSON 与 `tests/m9-product/core-capabilities.test.tsx`，单独提交
   `fix(product): normalize m9 core scene resource wrapper`；
2. shared `final-run` 当前无条件用 base Catalog 重建 ScenePackage，不能验证已由 Task 5/5A 正式
   绑定 project merged Catalog 的 M9 Scene。先在 `tests/project-check/final-run.test.ts` 加 Red，
   再让 checker 从 base 与 project-local generated Catalog 中选择唯一匹配全部 ready Scene task
   `resourceCatalogFingerprint` 的 current Catalog；零个或多个匹配都 fail closed。GPS 继续匹配
   原 base Catalog，generated bytes 不变。此修复只是让既有 checker 读取当前项目 authority，
   不提升视觉 capability，也不改变 Catalog writer 或 runtime。
