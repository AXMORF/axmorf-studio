...
export const PRODUCT_COMIC_SCENE_AUDIO_SPECS = [
  {meaningId: "problem-hook", fileName: "identity-break-pulse.wav", title: "Identity break pulse", durationSeconds: 0.34, frequencyHz: 196, overtoneHz: 294},
  {meaningId: "problem-friction", fileName: "drift-warning-tick.wav", title: "Drift warning tick", durationSeconds: 0.28, frequencyHz: 233, overtoneHz: 174},
  {meaningId: "product-reveal", fileName: "ink-close-snap.wav", title: "Ink close snap", durationSeconds: 0.3, frequencyHz: 330, overtoneHz: 495},
  {meaningId: "core-capabilities", fileName: "authority-stack-chime.wav", title: "Authority stack chime", durationSeconds: 0.42, frequencyHz: 262, overtoneHz: 392},
  {meaningId: "workflow-input", fileName: "source-check-tick.wav", title: "Source check tick", durationSeconds: 0.26, frequencyHz: 294, overtoneHz: 440},
  {meaningId: "workflow-create", fileName: "seal-lock-pulse.wav", title: "Seal lock pulse", durationSeconds: 0.36, frequencyHz: 247, overtoneHz: 370},
  {meaningId: "workflow-result", fileName: "evidence-stamp.wav", title: "Evidence stamp", durationSeconds: 0.32, frequencyHz: 220, overtoneHz: 440},
  {meaningId: "differentiated-value", fileName: "invalidation-crack.wav", title: "Invalidation crack", durationSeconds: 0.3, frequencyHz: 185, overtoneHz: 277},
  {meaningId: "proof-and-fit", fileName: "handoff-confirm.wav", title: "Handoff confirm", durationSeconds: 0.34, frequencyHz: 349, overtoneHz: 523},
  {meaningId: "call-to-action", fileName: "process-start-chime.wav", title: "Process start chime", durationSeconds: 0.4, frequencyHz: 294, overtoneHz: 587},
] as const;
...
const renderSceneCue = (spec: SceneAudioSpec): Buffer => {
  ...
  for (let index = 0; index < sampleFrames; index += 1) {
    ...
    const envelope = Math.sin(Math.PI * Math.min(1, attack)) * Math.pow(Math.max(0, release), 1.7);
    const fundamental = Math.sin(2 * Math.PI * spec.frequencyHz * time);
    const overtone = Math.sin(2 * Math.PI * spec.overtoneHz * time + Math.PI / 7);
    const transient = Math.sin(2 * Math.PI * (spec.overtoneHz * 1.7) * time) * Math.exp(-time * 19);
    const sample = Math.max(-1, Math.min(1, (fundamental * 0.66 + overtone * 0.24 + transient * 0.1) * envelope * 0.22));
...
const assertWaveFacts = (bytes: Buffer, spec: SceneAudioSpec) => {
  if (
    ...
    throw new Error(`Scene cue is not the fixed 48 kHz mono s16le PCM: ${spec.meaningId}.`);
  }
};

export const validateProductComicSceneAudio = async ({rootDir, overlay: rawOverlay, catalog: rawCatalog}: {readonly rootDir: string; readonly overlay: unknown; readonly catalog: unknown}) => {
  ...
  if (sceneDescriptors.length !== PRODUCT_COMIC_SCENE_AUDIO_SPECS.length) {
    throw new Error("Project Catalog must contain exactly ten Scene audio descriptors.");
  }
  const expected = readExpectedSceneDescriptors();
  for (const {spec, bytes: expectedBytes} of expected) {
    const descriptor = sceneDescriptors.find((candidate) => candidate.id === assetId(spec));
    if (!descriptor || descriptor.kind !== "asset" || descriptor.mediaRole !== "scene-sfx" || descriptor.assetKind !== "audio" || descriptor.license.verificationStatus !== "verified" || descriptor.localPath !== localPath(spec) || descriptor.media?.sampleRate !== SAMPLE_RATE || descriptor.media.codec !== "pcm_s16le") {
      throw new Error(`Scene audio descriptor is missing or invalid: ${spec.meaningId}.`);
    }
    ...
    if (
      ...
      throw new Error(`Scene audio bytes or checksum drifted: ${spec.meaningId}.`);
    }
    const catalogEntry = catalog.entries.find((entry) => entry.descriptor.id === descriptor.id);
    if (!catalogEntry || serializeCanonicalJson(catalogEntry.descriptor) !== serializeCanonicalJson(descriptor)) {
...
const run = async (mode: "write" | "check") => {
  ...
  const generatedCatalogPath = resolve(projectRoot, "generated/resource-catalog.generated.json");
  const baseCatalog = ResourceCatalogSchema.parse(
    JSON.parse(await readFile(resolve(rootDir, "src/remotion/catalog/resource-catalog.generated.json"), "utf8")),
  );
  const currentOverlay = ProjectOverlaySchema.parse(
    JSON.parse(await readFile(overlayPath, "utf8")),
  );
  if (currentOverlay.baseCatalogFingerprint !== baseCatalog.catalogFingerprint) {
    throw new Error("Project Catalog overlay points at a stale shared Catalog.");
  }
  const rendered = readExpectedSceneDescriptors();
  const sceneDescriptors = rendered.map(({spec, bytes}) => buildDescriptor(spec, bytes));
  ...
  const expectedOverlay = ProjectOverlaySchema.parse({
    ...currentOverlay,
    descriptors: [...otherDescriptors, ...sceneDescriptors].sort((left, right) =>
  ...
  if (mode === "write") {
    ...
    await writeAtomic(overlayPath, `${serializeCanonicalJson(expectedOverlay)}\n`);
    await writeAtomic(generatedCatalogPath, renderResourceCatalogJson(expectedCatalog));
  }
  ...
  process.stdout.write(`M9 Scene audio current: ${rendered.length} PCM cues.\n`);
};
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(import.meta.filename)) {
  ...
  if (mode !== "write" && mode !== "check") {
    ...
    run(mode).catch((error: unknown) => {
      process.stderr.write(`${error instanceof Error ? error.message : "Scene audio failed."}\n`);
      process.exitCode = 1;
    });
  }
}
