...
const ProductionReadabilityPolicyInputObject = z
  .object({
    schemaVersion: z.literal(1),
    policyId: z.literal(PRODUCTION_READABILITY_POLICY_ID),
...
const ProductionReadabilityPolicyInputSchema =
  ProductionReadabilityPolicyInputObject.readonly();

export const computeProductionReadabilityPolicyFingerprint = (
  rawPolicy: unknown,
) => {
  const record = { ...(rawPolicy as Record<string, unknown>) };
  delete record.policyFingerprint;
  const policy = ProductionReadabilityPolicyInputSchema.parse(record);
...
const buildProductionReadabilityPolicyInput = ({
  ...
  return ProductionReadabilityPolicyInputSchema.parse({
    schemaVersion: 1,
    policyId: PRODUCTION_READABILITY_POLICY_ID,
    policyVersion: 1,
    width: parsedWidth,
    height: parsedHeight,
    scale: {
...
export const ProductionReadabilityPolicySchema =
  ProductionReadabilityPolicyInputObject.extend({
    ...
    .superRefine((policy, context) => {
      ...
      if (
        ...
        policy.policyFingerprint !==
          computeProductionReadabilityPolicyFingerprint(expected)
      ) {
        context.addIssue({
          code: "custom",
          message:
...
export type ProductionReadabilityPolicy = z.infer<
  typeof ProductionReadabilityPolicySchema
>;
