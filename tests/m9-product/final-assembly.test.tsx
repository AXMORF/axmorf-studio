...
test("M9 FinalAssembly binds narrative Scene reference Catalog global and Composition identities", async () => {
  ...
  const sealed = SealedNarrationManifestSchema.parse(
    await readJson("src/projects/product-comic-vertical/generated/sealed-narration.generated.json"),
  );
  ...
  )) as {coverageFingerprint: string; entries: {packageFingerprint: string}[]};
  const catalog = ResourceCatalogSchema.parse(data.assemblyCatalog);
  assert.equal(assembly.narrativeReportFingerprint, data.narrativeReportFingerprint);
  assert.equal(assembly.sealedNarrationChecksum, sealed.completeAudio.checksum);
  assert.equal(assembly.sealedNarrationFingerprint, sealed.sealedNarrationFingerprint);
  ...
  assert.equal(assembly.rendererRegistryFingerprint, rendererRegistryFingerprint);
  assert.equal(assembly.resourceCatalogFingerprint, catalog.catalogFingerprint);
  assert.ok(
    catalog.entries.filter(({descriptor}) => descriptor.kind === "authoring-reference").length >= 3,
  );
  assert.equal(
