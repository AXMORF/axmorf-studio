...
test("FinalAssemblyPlan binds every ordered identity and exact runtime version", () => {
  ...
  assert.notEqual(
    ...
    createFinalAssemblyPlan({...finalAssemblyInput(), remotionVersion: "4.0.490"}).finalAssemblyFingerprint,
  );
  assert.notEqual(
    plan.finalAssemblyFingerprint,
    createFinalAssemblyPlan({...finalAssemblyInput(), durationInFrames: 121}).finalAssemblyFingerprint,
  );
});
