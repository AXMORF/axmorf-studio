...
test("M9 GlobalSound binds exactly two full-length project-authored PCM assets", async () => {
  ...
  assert.deepEqual(
    plan.assets.map(({ role }) => role),
    ["cross-scene-ambience", "global-bgm"],
  );
  assert.ok(
    plan.assets.every(
      ({ startFrame, endFrame }) => startFrame === 0 && endFrame === 5116,
    ),
  );
  ...
  for (const asset of plan.assets) {
    const descriptor = catalog.entries.find(
      ({ descriptor }) => descriptor.id === asset.resourceId,
    )?.descriptor;
    ...
    const facts = inspectCanonicalWav(
      await readFile(join(rootDir, asset.publicPath)),
    );
...
test("M9 ducking derives only from sealed absolute spoken ranges with clamp and minimum overlap", async () => {
  ...
  const ranges = createSpokenFrameRanges(
    timing.segments,
  ...
  const policy =
    productComicVerticalFinalAssemblyData.finalSound.plan.duckingPolicy;
...
test("M9 GlobalSound fails closed on Scene cues timing gain duration and checksum drift", () => {
  ...
  for (const changed of [
    ...
    {
      ...plan,
      masteringPolicy: { ...plan.masteringPolicy, narrationGain: 0.9 },
    },
    {
      ...plan,
