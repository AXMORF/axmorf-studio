...
export const collectDeliveryCoverSourceGraph = async ({
  ...
  const files: Array<
    DeliveryCoverPackage["sourceFiles"][number]
  > = [];
  ...
  const sourceFiles = files as unknown as DeliveryCoverPackage["sourceFiles"];
  return {
    files: sourceFiles,
    compositions,
    sourceGraphFingerprint: computeDeliveryCoverSourceGraphFingerprint({
      storyId,
      sourceFiles,
    }),
  } as const;
