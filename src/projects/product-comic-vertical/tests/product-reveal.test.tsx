...
const sceneRoot = "src/projects/product-comic-vertical/scenes/product-reveal";
...
test("product-reveal owns its fixed exact ScenePackage file set", async () => {
  ...
  assert.deepEqual(
    missing,
    [],
    `missing product-reveal contracts: ${missing.join(", ")}`,
  );
});
...
test("product-reveal Renderer and authoring boundaries are explicit", async () => {
  ...
  assert.match(
    renderer,
    /import\s*\{AdaptedShot\}\s*from\s*"\.\/shots\/video-shotcraft\/draw-svg-trace\/AdaptedShot"/u,
  );
...
test("product-reveal task plans and presealed cue form one fail-closed 233-frame bundle", async () => {
  ...
  assert.match(
    task.continuity.continuityBrief,
    /创作权威.*runtime projection/u,
  );
...
test("product-reveal exact lineage closure real frame binding and media evidence reproduce a pass-only receipt", async () => {
  ...
  assert.equal(
    exact.snapshotFingerprint,
    task.allowedSnapshots[0]?.snapshotFingerprint,
  );
  ...
  assert.equal(
    (
      item.exactDemo as {
        sourceFiles: readonly { path: string; checksum: string }[];
      }
  ...
  assert.deepEqual(binding, {
    importSpecifier: "./shots/video-shotcraft/draw-svg-trace/AdaptedShot",
  ...
  assert.deepEqual(
    reviewItem.phasePairs.map(
      ({ normalizedPhase, sourceFrame, adaptationFrame }) => ({
        normalizedPhase,
        sourceFrame,
        adaptationFrame,
      }),
  ...
  assert.equal(
    receipt.items[0]?.reviewItemFingerprint,
    reviewItem.itemFingerprint,
  );
  assert.equal(
    receipt.items[0]?.rendererBinding.importSpecifier,
    binding.importSpecifier,
  );
  assert.equal(
    receipt.items[0]?.rendererBinding.sceneFrameIdentifier,
    "sceneFrame",
  );
  assert.equal(
    receipt.items[0]?.rendererBinding.derivedFrameIdentifier,
    "shotFrame",
  );
});
...
test("product-reveal ScenePackage binds the current exact receipt Renderer and Scene-local sound only", async () => {
  ...
  assert.equal(
    scenePackage.selectionFingerprint,
