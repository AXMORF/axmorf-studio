...
const synthesizeSample = (spec: AssetSpec, sampleIndex: number) => {
  ...
  if (spec.role === "cross-scene-ambience") {
    ...
    const paperGrain =
      oscillator(91, sampleIndex, 0.2) *
      oscillator(0.31, sampleIndex, 1.2);
    const inkTable =
...
export const inspectCanonicalWav = (bytes: ArrayLike<number>) => {
  ...
  if (channels !== CHANNELS || sampleRate !== SAMPLE_RATE || bitsPerSample !== BITS_PER_SAMPLE) {
...
const buildTask8Inputs = async (rootDir: string) => {
  ...
  const [baseCatalog, currentOverlay, timing, render, sealed, narrative, coverage, packageJson] =
    await Promise.all([
      readJson(rootDir, "src/remotion/catalog/resource-catalog.generated.json").then(
        ResourceCatalogSchema.parse,
      ),
      readJson(rootDir, `src/projects/${STORY_ID}/resource-catalog.json`).then(
        ProjectOverlaySchema.parse,
      ),
      readJson(rootDir, `src/projects/${STORY_ID}/generated/semantic-timing.generated.json`).then(
        SemanticTimingSchema.parse,
      ),
      readJson(rootDir, `src/projects/${STORY_ID}/render.json`).then(RenderSpecSchema.parse),
      readJson(rootDir, `src/projects/${STORY_ID}/generated/sealed-narration.generated.json`).then(
        SealedNarrationManifestSchema.parse,
      ),
      readJson(rootDir, `src/projects/${STORY_ID}/generated/narrative-auto-check.generated.json`).then(
        NarrativeAutoCheckReportSchema.parse,
      ),
      readJson(rootDir, `src/projects/${STORY_ID}/generated/scene-coverage.generated.json`).then(
        z
          .object({
            coverageFingerprint: Sha256DigestSchema,
            entries: z.array(z.object({packageFingerprint: Sha256DigestSchema}).passthrough()),
          })
          .passthrough()
          .parse,
      ),
      readJson(rootDir, "package.json").then(
        z.object({dependencies: z.record(z.string(), z.string())}).passthrough().parse,
      ),
    ]);
  if (
  ...
  const byRole = new Map(descriptors.map((descriptor) => [descriptor.mediaRole, descriptor]));
  ...
  const globalVisualProjection = createGlobalVisualProjection({
    ...
    sourceChecksum: await checksumFile(
      join(rootDir, `src/projects/${STORY_ID}/global-visual/GlobalVisualLayers.tsx`),
    ),
  });
  ...
  const finalAssemblyInput = FinalAssemblyPlanInputSchema.parse({
    ...
    finalSoundProjectionFingerprint: finalSound.projection.projectionFingerprint,
    globalVisualPlanFingerprint: globalVisualPlan.planFingerprint,
    globalVisualProjectionFingerprint: globalVisualProjection.projectionFingerprint,
...
export const validateProductComicGlobalAudio = async ({rootDir}: {readonly rootDir: string}) => {
  const expected = await buildTask8Inputs(rootDir);
  const receipt = GlobalAudioReceiptSchema.parse(
    await readJson(rootDir, `src/projects/${STORY_ID}/generated/global-audio.generated.json`),
  );
  if (
