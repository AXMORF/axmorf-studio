...
>
> 状态：`AXMORF Studio`、内置 Preview Player、单 Workspace Root、Codex/Hermes 首批支持、Electron、macOS 13+、arm64/x64、App-running lifecycle、完整离线 Runtime DMG、GitHub Releases 站外分发、首阶段无签名和手动更新已确认；Phase A、Phase B、Phase C native gates 与 Phase D internal installer artifact 均 verified complete，公开发行许可 Gate 仍未满足
>
...
Phase D ordinary unsigned DMG、strict manifest/SBOM input、挂载/隔离安装验证和 manual-only 双架构 workflow 已在
exact hosted arm64 与真实 x64 runner 上完成，下载后的 complete dual artifact 也已通过 checksum、strict manifest、
exact file set 与 redaction 审计，状态为 `verified-complete`。即使 internal artifact Green，也不表示 Remotion
