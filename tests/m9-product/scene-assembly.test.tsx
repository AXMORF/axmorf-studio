...
test("M9 coverage and registry contain exactly ten current literal Scene bindings", async () => {
  ...
  for (const [index, meaningId] of meaningIds.entries()) {
    ...
    assert.equal(coverageEntry.packageFingerprint, scenePackage.packageFingerprint);
    assert.equal(
...
test("M9 visual and sound projections preserve sealed order timing and Scene ownership", async () => {
  ...
  assert.equal(productComicVerticalStoryVisualProjection.storyId, "product-comic-vertical");
  assert.equal(productComicVerticalStoryVisualProjection.durationInFrames, 5116);
  ...
  assert.deepEqual(
    productComicVerticalStoryVisualProjection.entries.map(({meaningId}) => meaningId),
    meaningIds,
  );
  assert.deepEqual(
    productComicVerticalStoryVisualProjection.entries.map(({startFrame, endFrame}) => ({
      startFrame,
      endFrame,
    })),
  ...
  assert.ok(
    productComicVerticalStoryVisualProjection.transitions.every(
      (transition) => transition.kind === "hard-cut" && transition.durationInFrames === 0,
    ),
  );
  assert.deepEqual(
    productComicVerticalSoundDesignProjection.entries.map(({meaningId}) => meaningId),
    meaningIds,
  );
  ...
  assert.deepEqual(Object.keys(productComicVerticalRendererPropsByMeaning), meaningIds);
});
...
test("M9 aggregate runtime is static and does not take over captions narration or global layers", async () => {
  ...
  assert.equal((registrySource.match(/^import Renderer\d+ from /gmu) ?? []).length, 10);
