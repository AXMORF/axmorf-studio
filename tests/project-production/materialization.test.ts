...
import { buildProducerTaskSpec, type ProducerTaskSpec } from "../../src/contracts";
import {
  commitTaskArtifact,
...
const commit = async (
  ...
  const attestation = (await commitTaskArtifact({ rootDir, task, workspace })).attestation;
...
test("owner materialization atomically replaces src/public/file roots and verifies live bytes", async (context) => {
  ...
  const global = buildProducerTaskSpec({
    ...
    declaredReadSet: [],
    declaredOutputSet: ["project/global-visual-plan.json", "src/GlobalVisualLayers.tsx"],
  ...
  const [sceneAttestation, globalAttestation] = await Promise.all([
    ...
    commit(rootDir, global, {
      ...
      "src/GlobalVisualLayers.tsx": "export const GlobalVisualLayers = () => null;",
    }),
  ]);
  const oldScene = join(rootDir, "src/projects/story-example/scenes/opening");
  const oldPublic = join(rootDir, "public/projects/story-example/scenes/opening");
  ...
  const sceneTaskInputs = new Map<string, unknown>([["opening", { meaningId: "opening" }]]);
  ...
  assert.equal(await readFile(join(oldScene, "Renderer.tsx"), "utf8"), "export default () => null;");
  assert.equal(await readFile(join(oldPublic, "texture.bin"), "utf8"), "texture-v1");
  assert.equal(
    await readFile(join(rootDir, "src/projects/story-example/global-visual-plan.json"), "utf8"),
    "{}\n",
  );
  ...
  await assert.rejects(
    () => verifyMaterializedOwnerArtifacts({ rootDir, projectId: "story-example", artifacts, sceneTaskInputs }),
    /checksum drifted/,
  );
});
...
test("cross-root promotion rolls back an already replaced owner root", async (context) => {
  ...
  const publicRoot = join(rootDir, "public/projects/story-example/scenes/opening");
  ...
  assert.equal(await readFile(join(sceneRoot, "old.txt"), "utf8"), "old-source");
  assert.equal(await readFile(join(publicRoot, "old.txt"), "utf8"), "old-public");
});

test("Scene materialization clears and verifies the public root when artifact has no public outputs", async (context) => {
  const rootDir = await mkdtemp(join(tmpdir(), "rsp-materialize-empty-public-"));
  context.after(() => rm(rootDir, { recursive: true, force: true }));
  const task = buildProducerTaskSpec({
    taskKind: "scene-owner",
    storyId: "story-example",
...
test("prepare-like fixed Scene files require an explicit exact manifest", async (context) => {
  const rootDir = await mkdtemp(join(tmpdir(), "rsp-materialize-fixed-scene-file-"));
  context.after(() => rm(rootDir, { recursive: true, force: true }));
  const task = buildProducerTaskSpec({
    taskKind: "scene-owner",
    storyId: "story-example",
  ...
  const artifacts = [{ task, attestation }];
  const sceneTaskInputs = new Map<string, unknown>([["opening", { meaningId: "opening" }]]);
  await materializeOwnerArtifacts({ rootDir, projectId: "story-example", artifacts, sceneTaskInputs });
  const sceneRoot = join(rootDir, "src/projects/story-example/scenes/opening");
  ...
  await assert.rejects(
    () => verifyMaterializedOwnerArtifacts({
      rootDir, projectId: "story-example", artifacts, sceneTaskInputs, additionalSceneFiles,
    }),
    /checksum drifted/,
  );
  await writeFile(fixedPath, fixedBytes);
...
for (const unsafeParent of ["src", "public"] as const) {
  test(`owner materialization rejects a symlinked ${unsafeParent} intermediate parent before writing outside`, async (context) => {
    const rootDir = await mkdtemp(join(tmpdir(), `rsp-materialize-${unsafeParent}-parent-`));
    const outside = await mkdtemp(join(tmpdir(), `rsp-materialize-${unsafeParent}-outside-`));
    context.after(() => rm(rootDir, { recursive: true, force: true }));
...
test("owner materialization rechecks parent containment immediately before promotion", async (context) => {
  ...
  const outside = await mkdtemp(join(tmpdir(), "rsp-materialize-parent-swap-outside-"));
  context.after(() => rm(rootDir, { recursive: true, force: true }));
  context.after(() => rm(outside, { recursive: true, force: true }));
  const task = buildProducerTaskSpec({
    taskKind: "scene-owner",
