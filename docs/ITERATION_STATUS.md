...
`v0.1.0` 后的 current source 已把 root README、两个 package README 与 creator template README 对齐为同一
Agent-first 入口：用户只提供 Workspace 位置和视频需求，Agent 随后读取当前安装版本的 README、`AGENTS.md`、Skill
与按阶段 references。README 不复制完整 production contract，生成 Workspace 的本地文档和结构化 CLI 继续拥有执行
authority。该 README 更新会随下一实际 package version 进入 npm registry；已发布的 `0.1.0` tarball 保持 immutable。
