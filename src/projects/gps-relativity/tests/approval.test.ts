...
} from "../tools/verification/approval";
...
test("approval artifact writer is pass-only atomic idempotent and checker is byte-exact", async (context) => {
  const temporaryRoot = await mkdtemp(
    join(tmpdir(), "gps-m8-approval-artifact-"),
  );
...
test("top-level check runs the complete static GPS verification profile", async () => {
