...
export const buildM9StillManifest = (): readonly StillEvidence[] => {
  ...
  return manifest.sort(
    (left, right) =>
      left.frame - right.frame || left.purpose.localeCompare(right.purpose),
  );
};
...
const renderReviewMedia = async (rootDir: string) => {
  ...
  if (manifest.length !== 45) {
    throw new Error(
      "M9 review manifest must contain exactly 45 ordered stills.",
    );
  }
...
if (
  ...
  if (mode === "render") {
    ...
    process.stderr.write(
      "Expected M9 Scene evidence mode: render|write|check.\n",
    );
    process.exitCode = 1;
  }
}
