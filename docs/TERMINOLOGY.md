...
| PublishingIntent            | Story 阶段一次创作并冻结的发布内容意图                         | future-only v1；绑定 current Story fingerprint，不重复 title，不保存 frame/timecode；包含 description、6–7 个唯一 topics、自由 collection 与 ordered chapters |
| CoverSpec                    | 双比例封面的固定代码图形约束                                   | 1600×1200 与 1200×1600 必须独立 Composition；禁止媒体、网络、远程字体、Scene/GlobalVisual 输出和机械裁切                                               |
| CoverAssignment             | 独立 Cover owner 的 future-only 冻结输入                        | v2 只嵌入 current StorySpec、VisualStyleSpec 和 fixed CoverSpec；不进入 production watcher/state                                                         |
| CoverResult                 | 已封存双比例 exact PNG 的 immutable result                      | v2 绑定 assignment/package/source graph、PNG checksum/大小/尺寸和缩略图检查；missing/stale 只阻止 delivery                                              |
| DeliverySpecification       | M10 本地交付 identity 声明                                     | v1 只读兼容；future-only v2 绑定 PublishingIntent、Cover result 与固定 archive policy，不拥有平台账号或网络配置                                        |
...
| PublishingMetadata          | 脚本确定性投影的 canonical 发布元数据                           | title/内容/timing/assembly/media 各取唯一 current authority；timecode 向下取整为 `HH:MM:SS`，不由 M10 Agent 改写                                        |
| DeliveryReleaseManifest     | M10 release 的机器可复验 identity 与文件清单                    | v1 只读兼容；v2 另绑定 PublishingIntent、Cover result、archive policy、payload checksum/大小/媒体参数与固定复验命令                                     |
