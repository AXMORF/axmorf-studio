...
import Renderer from "../scenes/workflow-create/Renderer";
import {
...
const sceneRoot = "src/projects/product-comic-vertical/scenes/workflow-create";
const audioPath =
...
test("workflow-create Renderer tells the implemented creation chain and remains visual-only", async () => {
  ...
  assert.match(
    frames[4] ?? "",
    /data-chain-state="sealed-static-evidence-bound"/u,
  );
...
test("workflow-create source excludes render-time integrations and CSS animation", async () => {
  ...
  assert.match(authoring, /<Sequence from=\{590\} durationInFrames=\{11\}/u);
  assert.doesNotMatch(authoring, /narration|global-bgm|cross-scene/iu);
  assert.match(root, /id="ProductComicVertical-workflow-create-Authoring"/u);
});
