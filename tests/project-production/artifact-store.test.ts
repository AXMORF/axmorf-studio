...
import { buildArtifactAttestation, buildProducerTaskSpec, serializeCanonicalJson } from "../../src/contracts";
import { commitTaskArtifact, inspectArtifact, inspectArtifactState } from "../../scripts/project-production/adapters/artifact-store";
...
test("Artifact Store reports stable typed invalidation states", async (context) => {
  ...
  await writeFile(join(malformed.artifactRoot, "artifact-attestation.json"), "{ nope");
  assert.equal(
    (await inspectArtifactState({ rootDir: malformed.rootDir, task })).artifactState,
    "manifest-invalid",
  );
  ...
  assert.equal(
    (await inspectArtifactState({ rootDir: identity.rootDir, task })).artifactState,
    "identity-mismatch",
  );
  ...
  assert.equal(
    (await inspectArtifactState({ rootDir: exactSet.rootDir, task })).artifactState,
    "exact-set-drift",
  );

  const checksum = await committedFixture("rsp-artifact-checksum-");
  context.after(() => rm(checksum.rootDir, { recursive: true, force: true }));
  await writeFile(join(checksum.artifactRoot, "files/public/cover.png"), "drift");
  assert.equal(
    (await inspectArtifactState({ rootDir: checksum.rootDir, task })).artifactState,
    "checksum-drift",
  );
  ...
  assert.equal(
    (await inspectArtifactState({ rootDir: unsafe.rootDir, task })).artifactState,
    "unsafe-path",
  );
});
