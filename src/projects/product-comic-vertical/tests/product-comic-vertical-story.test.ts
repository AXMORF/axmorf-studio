...
test("product story is a ten-beat authored Chinese narrative with fixed CTA", async () => {
  const [brief, story, narration, render, report, claims] = await Promise.all([
    ...
    readJson("narration.json").then((value) =>
      NarrationSpecSchema.parse(value),
    ),
  ...
  assert.ok(
    brief.targetDurationSeconds >= 120 && brief.targetDurationSeconds <= 180,
  );
  assert.deepEqual(
  ...
  assert.ok(
    chunks.every(({ ttsText }) => ttsText.length >= 16 && ttsText.length <= 72),
  );
  assert.ok(
    chunks.filter(({ ttsText }) => /。.+[。！？]$/.test(ttsText)).length >= 4,
  );
  assert.equal(
  ...
  for (const [index, binding] of claims.storyBindings.entries()) {
    ...
    assert.ok(
      binding.chunkBindings.every(({ claimIds }) => claimIds.length > 0),
    );
  }
  assert.equal(
...
test("project narration contains no mechanical punctuation splitter or emotion control policy", async () => {
  ...
  assert.ok(
    story.beats.some(({ explicitPauses }) => explicitPauses.length > 0),
  );
