...
import { buildGpsM8FrozenInputs } from "../tools/verification/final-inputs";
import {
...
test("GPS M8 frozen inputs bind current M1-M7 identities and project-local Catalog overlay", async () => {
  ...
  assert.doesNotThrow(() =>
    ResourceCatalogSchema.parse(frozen.assemblyCatalog),
  );
  assert.doesNotThrow(() =>
    GlobalSoundPlanSchema.parse(frozen.globalSoundPlan),
  );
  assert.doesNotThrow(() =>
    GlobalVisualPlanSchema.parse(frozen.globalVisualPlan),
  );
  ...
  for (const reference of frozen.globalSoundPlan.assets) {
    ...
    assert.equal(
      reference.descriptorFingerprint,
      catalogEntry.descriptorFingerprint,
    );
...
test("M8 check mode is byte-exact and never rewrites frozen artifacts", async () => {
  ...
  const before = await Promise.all(
    paths.map((path) => readFile(join(rootDir, path))),
  );
  const frozen = await buildGpsM8FrozenInputs(rootDir);
