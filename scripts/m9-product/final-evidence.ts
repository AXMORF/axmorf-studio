...
const EVIDENCE_PATH =
  `src/projects/${STORY_ID}/generated/final-preview-evidence.generated.json`;
const REVIEW_PATH =
  `src/projects/${STORY_ID}/reviews/final-assembly-review.json`;
...
export const M9_REVIEW_FRAMES = [...new Set([
  0,
  ...buildM9StillManifest().map(({frame}) => frame),
  5115,
...
export const readCurrentM9ReviewIdentities = async (rootDir: string) => {
  ...
  const input = {
    ...
    finalAssemblyFingerprint:
      assembly.finalAssembly.finalAssemblyFingerprint,
    resourceCatalogFingerprint:
      assembly.assemblyCatalog.catalogFingerprint,
...
export const inspectM9FinalPreviewTechnical = async ({
  ...
  if (
    ...
    throw new Error("M9 final preview stereo channels are missing or phase-cancelled.");
  }
  ...
  const measurementFrames = [0, 6, 15, 302, 303, 574, 575, 579, 580, 5100, 5115] as const;
...
export const renderM9ContactSheet = async (rootDir: string) => {
  ...
  for (let index = 0; index < blankCount; index += 1) {
    filters.push(
      `color=c=#171117:s=216x384:d=1[blank${index}]`,
    );
  }
...
if (
  ...
  const run = async () => {
    if (process.argv.length !== 3) {
      throw new Error("Expected exactly render, contact-sheet, write, or check.");
    }
    if (mode === "render") {
